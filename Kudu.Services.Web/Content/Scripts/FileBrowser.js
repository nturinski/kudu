...
$.connection.hub.start().done(function () {
    ...
    function _processEntry(entry, parentPath) {
        ...
        if (entry.isFile) {
            ...
            var totalFilesDeferreds = [];
            var directoryCallback = function (entries) {
                if (entries.length == 0) {
                    whenArray(totalFilesDeferreds).done(function () { deferred.resolveWith(null, arguments) });
                } else {
                    // readEntries might return more entries
                    reader.readEntries(directoryCallback);

                    var directoryPath = parentPath + '/' + entry.name;
                    Array.prototype.push.apply(totalFilesDeferreds, $.map(entries, function (e) {
